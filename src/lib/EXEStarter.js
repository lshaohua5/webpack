...
/*
* version:  3.8.1.0
* author :  腾讯云官方团队-kamis
* info   :  EXEStarter.js 用来给web和本地exe进行通信的类库。
* items  :  1. 支持三种类型标签 LiveRoom 、RtcRoom 、 PushPlay
*           2. 默认使用http协议的业务网站，
*           3. 对于https协议的业务网站：
*               a.参考《https站点配置步骤.docx》"部署本地SSL证书，
*               b.使用EXEStarter SDK时，调用startEXE接口设置httpsSSLPort的端口。
*/
...
var LiveRoom = {
     ...
     *     https_SSLPort:  Int   如果您是Https站点,参考《https站点配置步骤.docx》配置SSL证书和绑定端口，并把SSL绑定的端口从此处传入
     *  },
...
var PushPlay = {
     ...
     * }
     * 注:PC截图已经是无损压缩，清晰度依赖原始图片的清晰度，一般player经过编码网络传输损失，原始图比较模糊。
     * 如player是小程序端:提升座席对小程序截图质量，需改小程序live-push参数配置: minBit: 400,  maxBit: 800,  aspectT: "3:4"。
     */
...
LiveRoom = (function () {
    ...
    function startEXE(object) {
        ...
        //判断https站点参数: 1. 必须传入ssl_port  2. 必须单实例模式
        if (isHttpsProtocal() == 1)
        {
            roomInfo.httpPort = (object.custom && object.custom.https_SSLPort) || 0;
            if (roomInfo.httpPort == 0)
            {
                console.error("RtcRoom startEXE参数错误");
                alert("RtcRoom::Https站点必须配置https_SSLPort,请参考《SSL配置》。")
                object.fail && object.fail({ errCode: -9999, errMsg: "RtcRoom.startEXE object参数错误" });
                return;
            }
            custom.singleton = true;
        }
        else if (custom.singleton) {
        ...
        starterReportInfo.int32_sdkapp_id = accountInfo.sdkAppID;
        ...
        if (custom.singleton == false) {
            ...
            ExeMsgChannel.startConnect({
                data: {
                    serverDomain: wsiteProtocolHeader + roomInfo.httpPort.toString() + "/query",
    ...
    function stopEXE(object) {
        ...
        if (index == -1) { return; }
        //通知断开连接：
        var cmdUrl = wsiteProtocolHeader + arrExeRoom[index].httpPort.toString() + "/quit";
                        ...
                        if (cmd.event == "roomStatus") {
                            ...
                            if (cmd.code == 0) {
                                ...
                                event.onRoomClose && event.onRoomClose({
                                    data: {
                                        roomID: room.roomID, roomName: room.roomName, roomAction: room.roomAction,
                    ...
                    if (ret.code == 0 && ret.port == room.httpPort) {
                        if (bTestConnectExe) {
                            room.roomCB && room.roomCB.fail && room.roomCB.fail({
                                errCode: -2, errMsg: "单实例模式下已启动exe应用实例，请检查应用是否完全关闭!"
                            });
                        }
                        else {
                            ...
                            if (room.bSendParams == true) {
                                ...
                                var cmdUrl = wsiteProtocolHeader + room.httpPort.toString() + "/externalProtol?json=" + params;
...
PushPlay = (function () {
    ...
    function startEXE(object) {
        ...
        //判断https站点参数: 1. 必须传入ssl_port  2. 必须单实例模式
        if (isHttpsProtocal() == 1) {
            session.httpPort = (object.custom && object.custom.https_SSLPort) || 0;
            if (session.httpPort == 0) {
                console.error("PushPlay startEXE参数错误");
                alert("PushPlay::Https站点必须配置https_SSLPort,请参考《SSL配置》。")
                object.fail && object.fail({ errCode: -9999, errMsg: "PushPlay.startEXE object参数错误" });
                return;
            }
        }
    ...
    function doHttpCreateSession(session_t) {
        var cmdUrl = wsiteProtocolHeader + session_t.httpPort.toString() + "/csliveCreateSession?";
        ...
        var paramTitle = encodeURIComponent("title") + "=" + encodeURIComponent(session_t.title);
        cmdUrl = cmdUrl + paramPushUri + '&' + paramPullUri + '&' + paramTitle;
    ...
    function reStartExe(session_t) {
        ExeMsgChannel.startConnect({
            data: {
                serverDomain: wsiteProtocolHeader + session_t.httpPort.toString() + "/query",
    ...
    function stopEXE() {
        ...
        var cmdUrl = wsiteProtocolHeader + session.httpPort.toString() + "/csliveDestroySession";
    ...
    function addPusher(object) {
        ...
        if (object && object.data.url) {
            var cmdUrl = wsiteProtocolHeader + session.httpPort.toString() + "/csliveAddPusher?";
            var param = encodeURIComponent("url") + "=" + encodeURIComponent(object.data.url);
            cmdUrl = cmdUrl + param;
            ExeMsgChannel.sendCmd({ data: { cmdUrl: cmdUrl + "&", }, });
        }
    };

    function delPusher(object) {
        if (bConnectExe == false) { return; }
        if (object && object.data.url) {
            var cmdUrl = wsiteProtocolHeader + session.httpPort.toString() + "/csliveDelPusher?";
            var param = encodeURIComponent("url") + "=" + encodeURIComponent(object.data.url);
            cmdUrl = cmdUrl + param;
            ExeMsgChannel.sendCmd({ data: { cmdUrl: cmdUrl + "&", }, });
        }
    ...
    function videoSnapshot(object) {
        ...
        if (object && object.data.userRole) {
            if (object.data.userRole == "pusher") {
                var cmdUrl = wsiteProtocolHeader + session.httpPort.toString() + "/csliveSnapshotPusher";
                console.log('snapshot-url:' + cmdUrl);
                ExeMsgChannel.sendCmd({ data: { cmdUrl: cmdUrl + "?", }, });
            }
            else if (object.data.userRole == "player") {
                var cmdUrl = wsiteProtocolHeader + session.httpPort.toString() + "/csliveSnapshotPlayer";
                console.log('snapshot-url:' + cmdUrl);
                ExeMsgChannel.sendCmd({ data: { cmdUrl: cmdUrl + "?", }, });
            }
        }
    ...
    function setMute(object) {
        if (bConnectExe == false) { return; }
        if (object && object.data.bMute) {
            var cmdUrl = wsiteProtocolHeader + session.httpPort.toString() + "/csliveSetMute?";
            console.log('setMute:' + object.data.bMute);
            var param = encodeURIComponent("setMute") + "=" + object.data.bMute;
            cmdUrl = cmdUrl + param;
            ExeMsgChannel.sendCmd({ data: { cmdUrl: cmdUrl + "&" }});
        }
    }
                ...
                onConnectStatus: function (ret) {
                    if (ret.code == 0 && ret.port == session.httpPort) {
                        ...
                        if (session.bSendParams == true) {
                            ...
                            var cmdUrl = wsiteProtocolHeader + session.httpPort.toString() + "/externalProtol?json=" + params;
...
var wsiteProtocolHeader = "http://127.0.0.1:";
function isHttpsProtocal(){
    var ishttps = 'https:' == document.location.protocol ? true : false; ishttps
    ishttps=false;
    if (ishttps) {
        wsiteProtocolHeader = "https://127.0.0.1:";
        return 1;
    } else {
        wsiteProtocolHeader = "http://127.0.0.1:";
        return 0;
    }
}
/***********************************************************************************
